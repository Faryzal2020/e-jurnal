...
?>
	<div id="calender_section">
		<h2>
        	<a href="javascript:void(0);" title="bulan sebelumnya" onclick="getCalendar('calendar_div','<?php echo date("Y",strtotime($date.' - 1 Month')); ?>','<?php echo date("m",strtotime($date.' - 1 Month')); ?>');"><span class="glyphicon glyphicon-backward" style="pointer-events: none;"></a>
            <select name="month_dropdown" title="pilih bulan" class="month_dropdown dropdown"><?php echo getAllMonths($dateMonth); ?></select>
			<select name="year_dropdown" title="pilih tahun" class="year_dropdown dropdown"><?php echo getYearList($dateYear); ?></select>
            <a href="javascript:void(0);" title="bulan selanjutnya" onclick="getCalendar('calendar_div','<?php echo date("Y",strtotime($date.' + 1 Month')); ?>','<?php echo date("m",strtotime($date.' + 1 Month')); ?>');"><span class="glyphicon glyphicon-forward" style="pointer-events: none;"></a>
        </h2>
			...
			<?php 
				...
				for($cb=1;$cb<=$boxDisplay;$cb++){
					if(($cb >= $currentMonthFirstDay+1 || $currentMonthFirstDay == 7) && $cb <= ($totalDaysOfMonthDisplay)){
						...
						$kalendersql = "SELECT id_jurnal FROM jurnal WHERE date(waktu_selesai) >= '".$currentDate."' AND date(waktu_mulai) <= '".$currentDate."' AND nip='".$_SESSION['nip']."'";
