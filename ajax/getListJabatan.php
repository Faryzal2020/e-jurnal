	...
	if(mysqli_num_rows($result) > 0){
		...
		while($data = mysqli_fetch_array($result)){
			echo "<tr style=\"border-bottom: 2px solid #ECECEC;\">
					<td style=\"padding: 10px;\"'><span onclick=\"toggleChild('$data[0]','n','$data[1]')\">$data[1]</span></td> <td style='background-color: white; border: 1px solid white; width: 200px;'> <a class=\"EJBbtn pencetan\" onclick=\"editJabatan('$data[0]','$data[1]')\">Edit</a> <a class=\"EJBbtn pencetan\" onclick=\"lihatPegawai('$data[0]')\">Lihat Pegawai</a></td>
				 </tr>";
